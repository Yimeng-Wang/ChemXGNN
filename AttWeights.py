...
def eval_AttWeights(args):
    ...
    for iter, batch_data in enumerate(test_loader):
        if iter in index_to_explain:
            ...
            pred, node_weight = model.forward(bg, n_feats, e_feats, get_node_weight=True)
            min_value = torch.min(node_weight[1])
            max_value = torch.max(node_weight[1])
            node_weights = (node_weight[1] - min_value) / (max_value - min_value)
            node_weight = torch.sigmoid(node_weight[1])
            prob_aro = node_weight.detach().cpu().numpy().flatten().tolist()
            ...
            pred_unimp = model.forward(bg, n_feats_masks, e_feats, get_node_gradient=False)
            ...
            pred_imp = model.forward(bg, n_feats_masks_un, e_feats, get_node_gradient=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model', default='AttentiveFP',
