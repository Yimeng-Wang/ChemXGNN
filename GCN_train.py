...
def run_an_eval_epoch(args, model, data_loader,loss_criterion):
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            masks = masks.to(device)
            n_feats = bg.ndata.pop('hv').to(device)
            vali_prediction = model(bg, n_feats)
...
def train(args):
    ...
    model = GCNPredictor(in_feats=n_feats,
                         ...
                         activation=[F.relu] * args['num_layers'],
                         dropout=[args['dropout']] * args['num_layers'],
...
if __name__ == '__main__':
    import argparse
    from configure_model import gcn_configure
    ...
    args.update(gcn_configure(args))
