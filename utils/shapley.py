...
from torch_geometric.data import Data as Data2
...
class MarginalSubgraphDataset():

    def __init__(self, data, edge_index, exclude_mask, include_mask, subgraph_build_func):
        self.data=data
        self.smiles, self.graph, self.label, _ = self.data
        self.X = self.graph.ndata['hv']
        self.num_nodes = self.X.shape[0]
        self.edge_index = edge_index
        self.device = self.X.device
        self.exclude_mask = torch.tensor(exclude_mask).to(self.device)
        self.include_mask = torch.tensor(include_mask).to(self.device)
    ...
    def __getitem__(self, idx):
        ...
        exclude_data = Data2(x=exclude_graph_X, edge_index=exclude_graph_edge_index)
        include_data = Data2(x=include_graph_X, edge_index=include_graph_edge_index)
        return exclude_data, include_data


def marginal_contribution(data: Data, edge_index, exclude_mask: np.array, include_mask: np.array,
    ...
    marginal_subgraph_dataset = MarginalSubgraphDataset(data, edge_index,exclude_mask, include_mask, subgraph_build_func)
    dataloader = DataLoader(marginal_subgraph_dataset, batch_size=256, shuffle=False)
...
def mc_l_shapley(coalition: list,data: Data,  edge_index, local_raduis: int,
                 value_func: str, subgraph_building_method='zero_filling',
                 sample_num=100) -> float:
    """ monte carlo sampling approximation of the l_shapley value """
    smiles, bg, labels, _ = data
    graph = dgl.to_networkx(bg, node_attrs=['hv'])
    graph = nx.Graph(graph)
    num_nodes = bg.ndata['hv'].shape[0]
    subgraph_build_func = get_graph_build_func(subgraph_building_method)
    ...
    include_mask = np.stack(set_include_masks, axis=0)
    marginal_contributions = \
        marginal_contribution(data, edge_index, exclude_mask, include_mask, value_func, subgraph_build_func)
...
def gnn_score(coalition: list, data: Data, edge_index,value_func: str,
    ...
    mask_data = Data2(x=ret_x, edge_index=ret_edge_index)
...
def NC_mc_l_shapley(coalition: list, data: Data, local_raduis: int,
                    value_func: str, node_idx: int=-1, subgraph_building_method='zero_filling', sample_num=100) -> float:
