...
class MoleculeCSVDataset(object):
    ...
    def __init__(self, df, smiles_to_graph, node_featurizer, edge_featurizer, smiles_column,
                 features_column, cache_file_path, task_names=None, load=False, log_every=1000,
                 init_mask=True,n_jobs=1):
        self.df = df
        self.smiles = self.df[smiles_column].tolist()
        self.features = self.df[features_column].to_numpy().tolist()
    ...
    def __getitem__(self, item):
        ...
        if self.mask is not None:
            ...
            return self.smiles[item], self.graphs[item], self.features[item],self.labels[item], self.mask[item]
        else:
            #self.features[item] = torch.tensor(self.features[item])
            return self.smiles[item], self.graphs[item], self.features[item],self.labels[item]
