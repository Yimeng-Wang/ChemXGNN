import torch
from utils.configures_shap import mcts_args
from utils.shapley import mc_shapley, l_shapley, mc_l_shapley, gnn_score
...
class MCTS():
    def __init__(self, data,edge_index: torch.Tensor, n_rollout: int,
                 min_atoms: int, c_puct: float, expand_atoms: int, score_func):
        """ graph is a networkX graph """
        self.data = data
        self.smiles, self.bg, self.labels, _ = self.data
        self.X=self.bg.ndata['hv']
        self.edge_index = edge_index
        self.datas=Data2(x=self.X, edge_index=self.edge_index)
        self.graph = dgl.to_networkx(self.bg, node_attrs=['hv'])
        ...
        self.MCTSNodeClass = partial(MCTSNode, data=self.data, ori_graph=self.bg, c_puct=self.c_puct)
    ...
    def mcts_rollout(self, tree_node):
        ...
        if len(tree_node.children) == 0:
            ...
            scores = compute_scores(self.score_func,self.edge_index, tree_node.children)
...
def compute_scores(score_func,edge_index, children):
    results = []
    for child in children:
        if child.P == 0:
            score = score_func(child.coalition,child.data,edge_index)
        else:
            score = child.P
